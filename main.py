...
def robust_train(net, optimizer, trainloader, n_classes, config, train_loss_meter, train_accuracy_meter,
    ...
    # normalized_weight = (weights-min(weights))/(max(weights)-min(weights)) if (max(weights)-min(weights)) !=0 else weights/max(weights)

    # prediction = torch.zeros(n_samples,dtype=torch.bool).cuda()
    true = torch.zeros(n_samples,dtype=torch.bool).cuda()
    selected = torch.zeros(n_samples,dtype=torch.bool).cuda()
    unselected = torch.zeros(n_samples, dtype=torch.bool).cuda()
    ...
    for it, sample in enumerate(pbar):
        ...
        GT = sample['label_true']
        true[indices] = (GT.cuda() ==y)
        ...
        if train_acc0[0] !=0 and ((train_acc1[0]-train_acc0[0])/train_acc0[0] < config.tau):
            ...
            idx_selected = (((prediction0 == prediction1) & (prediction0 == y)) & (prediction1 == y))

        idx_unselected = (prediction0 != y) & (prediction1 == y)

        selected[indices] = idx_selected.cuda()

        unselected[indices] = idx_unselected.cuda()
        ...
        loss0 = (F.cross_entropy(logits0[idx_selected], y[idx_selected], reduction="none") * (
        ...
        loss_CR = F.cross_entropy(logits_CR, pesudo)
        weight = 0.8
...
def warmup(net, optimizer, trainloader, train_loss_meter, train_accuracy_meter, train_loss_meter1,
    ...
    for it, sample in enumerate(pbar):
        ...
        px0 = logits.softmax(dim=1)
        prediction0 = px0.max(dim=1)[1]

        # warmup net1
        outputs = net[1](x)
        logits = outputs['logits'] if type(outputs) is dict else outputs
        loss1 = F.cross_entropy(logits, y)
        # penalty = conf_penalty(logits)
        train_acc1 = accuracy(logits, y, topk=(1,))
        # loss1 = loss_ce + penalty
        px1 = logits.softmax(dim=1)
        prediction1 = px1.max(dim=1)[1]
...
if __name__ == '__main__':
    ...
    for epoch in range(config.epochs):
        ...
        if epoch < config.warmup_epochs and config.method not in ["small_loss", "co_teaching"]:
            ...
            robust_train(
                model, optim, input_loader, n_classes,
                config, train_loss_meter,
                train_accuracy_meter, train_loss_meter1,
                train_accuracy_meter1, corrected_label_distributions, ema_label_distributions, weight)

        eval_result = evaluate_cls_acc(loader_dict['test_loader'], model, device)
        test_accuracy = eval_result['accuracy']

        logger.info(
                f'>>  Epoch: {epoch} | Net [0] loss: {train_loss_meter.avg:.2f} | train acc: {train_accuracy_meter.avg:.2f} | test acc: {test_accuracy[0]:.2f} | Net [1] loss: {train_loss_meter1.avg:.2f} | train acc: {train_accuracy_meter1.avg:.2f} | test acc: {test_accuracy[1]:.2f} .')
